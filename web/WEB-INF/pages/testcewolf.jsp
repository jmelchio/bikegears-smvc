<%@page contentType="text/html"%>
<%@page pageEncoding="UTF-8"%>
<%@ include file="/WEB-INF/pages/fragments/taglibs.jspf" %>

<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN"
"http://www.w3.org/TR/html4/loose.dtd">

<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <%@ include file="/WEB-INF/pages/fragments/headSection.jspf" %>
        <title><spring:message code="application.title"/></title>
    </head>
    <jsp:useBean id="pieChartProducer" class="com.nob.bg.web.chart.cewolf.TestDatasetProducer" scope="request"/>
    <jsp:useBean id="pieChartPostProcessor" class="com.nob.bg.web.chart.cewolf.PieChartPostProcessor" scope="request"/>
    <body>
        <div id="container">
            <%@ include file="/WEB-INF/pages/fragments/header.jspf" %>
            <%@ include file="/WEB-INF/pages/fragments/menu.jspf" %>
            <div id="content">
                <h2><spring:message code="bike.entry.header"/></h2>
                <cewolf:chart 
                    id="pieChart" 
                    title="Pie Chart" 
                    type="pie"
                    showlegend="false">
                    <cewolf:data>
                        <cewolf:producer id="pieChartProducer"/>
                    </cewolf:data>
                    <cewolf:chartpostprocessor id="pieChartPostProcessor"/>
                </cewolf:chart>
                <p>
                    <cewolf:img chartid="pieChart" renderer="cewolf" width="400" height="200"/>
                </p>
            </div>
            <%@ include file="/WEB-INF/pages/fragments/footer.jspf" %>
        </div>
    </body>
</html>
